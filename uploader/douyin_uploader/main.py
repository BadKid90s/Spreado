...
async def douyin_cookie_gen(account_file):
    ...
    async with async_playwright() as playwright:
        ...
        chrome_path = get_chrome_path()
        if chrome_path:
            douyin_logger.info(f'[+] 使用自动检测到的Chrome浏览器: {chrome_path}')
...
class DouYinVideo(object):
    def __init__(self, title: str, content: str, tags: List[str], file_path: str | Path, publish_date: datetime,
        ...
        if self.local_executable_path:
            douyin_logger.info(f'[+] 自动检测到Chrome浏览器路径: {self.local_executable_path}')
        else:
            douyin_logger.warning('[!] 未检测到Chrome浏览器，将使用Playwright默认浏览器')
