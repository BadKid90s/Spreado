...
async def cookie_auth(account_file):
    async with async_playwright() as playwright:
        ...
        try:
            await page.wait_for_selector('div.title-name:has-text("微信小店")', timeout=1000)  # 等待1秒
...
async def get_tencent_cookie(account_file):
    ...
    async with async_playwright() as playwright:
        ...
        chrome_path = get_chrome_path()
        if chrome_path:
            tencent_logger.info(f'[+] 使用自动检测到的Chrome浏览器: {chrome_path}')
...
class TencentVideo(object):
    ...
    def __init__(self, title: str, content: str, tags: List[str], file_path: str | Path, account_file: str | Path,
        ...
        if self.local_executable_path:
            tencent_logger.info(f'[+] 自动检测到Chrome浏览器路径: {self.local_executable_path}')
        else:
            tencent_logger.warning('[!] 未检测到Chrome浏览器，将使用Playwright默认浏览器')
