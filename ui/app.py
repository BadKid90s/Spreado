...
# 主发布函数（生成器版本，用于实时日志）
def publish_video_generator(video_file_obj, thumbnail_file_obj, title, description, tags, scheduled_time, platforms):
    """主发布函数（生成器版本，用于实时日志）"""
    if not video_file_obj:
        yield "❌ 请先上传视频文件"
        return
        ...
        if platform_value in platform_display_mapping:
            ...
            try:
                ...
                if marker_index != -1:
                    ...
                    # 添加最新的平台结果（去掉开头的重复信息）
                    platform_lines = last_partial_result.split('\n')
                    # 找到平台相关信息的开始位置
                    start_index = 0
                    for i, line in enumerate(platform_lines):
