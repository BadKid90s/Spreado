...
class ShiPinHaoUploader(BaseUploader):
    ...
    async def _set_thumbnail(self, page: Page, thumbnail_path: Optional[str | Path]) -> bool:
        ...
        await page.wait_for_selector('div.single-cover-uploader-wrap div.text-wrap:has-text("上传封面")', timeout=5000)
        ...
        try:
            ...
            # 等待裁剪对话框出现
            await page.wait_for_selector('div.weui-desktop-dialog__wrp:has(h3:has-text("裁剪封面图"))', state='visible', timeout=10000)
        ...
        try:
            ...
            # 等待模态框关闭
            await page.wait_for_selector('div.weui-desktop-dialog__wrp:has(h3:has-text("裁剪封面图"))', state='hidden', timeout=10000)
        ...
        try:
            # 等待封面图片加载完成
            await page.wait_for_selector('div.vertical-cover-wrap img.cover-img-horizontal[src]', timeout=10000)
            
            # 获取封面图片的URL
            cover_image_url = await page.locator('div.vertical-cover-wrap img.cover-img-horizontal').get_attribute('src')
            if cover_image_url:
                self.logger.info(f"[+] 视频封面设置完成！封面图片URL: {cover_image_url[:50]}...")
                return True
            else:
